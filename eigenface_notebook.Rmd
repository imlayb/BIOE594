...
gbmFit1 <- train(Sex ~ ., data = trainTransformed, 
                 method = "gbm", 
...
```{r automatic models,eval=FALSE}
source("generateModels.R")
fitControl <- trainControl(## 10-fold CV
                           method = "repeatedcv",
                           number = 2,
                           ## repeated ten times
...
x<-generateModels(NZV_dat_TR[1:10,],list(Sex=meta_TR[1:10,2]),methods=methods,controls=controls)
```
