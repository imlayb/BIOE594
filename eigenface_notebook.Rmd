...
```{r plot eigen faces,echo=FALSE,fig.height=6}
par(mfrow = c(2, 5))
par(mar = c(0, 0, 4.1, 0))
for(i in seq(1:10)) {
...
The actual performance of the models were lackluster. The accuracies milled around the 70% mark, which was only marginally better than the majority classifier for gender. The sensitivity was essentially coin toss at 50% for each model. That is, they had a 50% chance of accurately predicting a female when they predicted female. The very poor performance of these models is likely the result of too many features and lack of relation between features. Pixels in isolation are not very informative of attributes such as gender.
...
Since using a cropped face vector did not yield great results, my second approach was to use each images projections in the first 100 principal compenents as features. Though I still remove NZV pixels to enable scaling of each pixel feature. As we have seen in the SVD plots, the eigenfaces appear to sometimes catch gender associated traits such as shading around where long hair might be.
...
The cumulative gains plot begin to exemplify how using eigenfaces improved the model's performances. Here we can see that XGB and C5.0 were remarkably accurate among their highest confidence predictions. This trend is different than for the NZV pre-processed models. The feature importance plot for C5.0 appeared to show that the model did not handle so many features very well. 100 features was likely a more comfortable place for C5.0, which is a tree based method. This plot helps confirm that XGB is a robust classifier with its mastery of hyperparameter tuning. 100 features may still have been too many for FDA.
...
Using the first 100 most important principal components afforded an appreciable performance increase. This makes sense as each component considers the entirety of each image. The major limitation of using pixels directly is how weak each feature is. One pixel may be important in the classification of some cases, but the neighbor pixel may be important for some others. There is no way for the classifier to make those associations when treating each image as a vector of pixel values. 
...
Below are confusion matrices and performance metrics for all models. The accuracy is only barely higher than the XGB and C5.0 models. I found this result to be spectacular news for both my efforts and the cognitive services API. Though my models are not robust to color images, different size images, non-centered images, and images with objects besides faces, their parity with the state-of-the-art model from Microsoft is reassuring. Simply extracting 100 principal components from a data set is sufficient to achieve most of the performance of an enterprise classifier - at least when it comes to gender.
...
Through the variety of pre-processing methods, classifiers, and the Azure API, I believe that I have achieved the objectives of this project. I observed appreciable improvements as I moved through the different pre-processing methods. The improvement mirrored the progress of the facial recognition field at large. I started with direct classifiers, I moved on to simple dimension reduction, and I finished with a modern implementation.

The lift plots also enabled visualization of how 
