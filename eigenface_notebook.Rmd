...
class<-"sex"
training<-data.frame(Class=meta_TR[[class]],NZV_dat_TR)
trainIndex <- createDataPartition(training[[class]], p = .3, 
                                  list = FALSE, 
                                  times = 1)
...
```{r model generation example,eval=FALSE}
...
```{r model analysis}
load(file.path("models","sex_models.Rdata"))
imp<-varImp(rf_fit,scale=TRUE)$importance
...
image(
    ...
    axes = FALSE,
    main="Important variables for sex classification")
